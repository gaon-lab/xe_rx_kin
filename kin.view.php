...
class kinView extends kin
{
	...
	function dispKinView()
	{
		...
		if(!$oDocument->isExists()) return new baseObject(-1, 'msg_document_is_null');
	...
	function dispKinWrite()
	{
		...
		if(!$oDocument->isExists())
		{
			if($user_point < $min_point)
			{
				...
				{
					return new baseObject(-1,'msg_limit_write');
				}
				else
				{
...
$oModuleModel = getModel('module');
	...
	function dispKinModifyReply()
	{
		...
		if(!$oReply->isExists() || !$oReply->isGranted()) return new baseObject(-1,'msg_not_permitted');
