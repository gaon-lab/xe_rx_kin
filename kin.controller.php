...
class kinController  extends kin {
	...
	function procKinInsert() {
		...
		if(!$this->grant->write_document) return new baseObject(-1, 'msg_not_permitted');
		...
		if(!$obj->title) return new baseObject(-1, 'msg_title_is_null');
		if(!$obj->content) return new baseObject(-1, 'msg_content_is_null');
		...
		if(!$oDocument->isExists()) {
			...
			if($give_point > $user_point) return new baseObject(-1,sprintf(Context::getLang('msg_point_is_over'), $user_point));
			if($this->module_info->limit_write == 'Y' && $give_point > $max_point) return new baseObject(-1,sprintf(Context::getLang('msg_point_limited'), $max_point));
			if($this->module_info->limit_write == 'Y' && $give_point < $min_point) return new baseObject(-1,sprintf(Context::getLang('msg_point_shortage'), $min_point));
	...
	//	if($oDocument->getCommentCount()>0) return new baseObject(-1,'msg_invalid_request');
	...
	function procKinSelectReply() {
		...
		if(!$oSourceDocument->isExists() || $oKinModel->getSelectedReply($oSourceDocument->document_srl))  return new baseObject(-1,'msg_invalid_request');

		$logged_info = Context::get('logged_info');
		if($oSourceDocument->get('member_srl')!=$logged_info->member_srl && !$logged_info->is_admin)  return new baseObject(-1,'msg_invalid_request');
		...
		if($oSourceDocument->get('member_srl') == $oComment->get('member_srl')) return new baseObject();
	...
	function procKinInsertReply() {
		...
		if(!$this->grant->write_reply) return new baseObject(-1, 'msg_not_permitted');

		$document_srl = Context::get('document_srl');
		$oSourceDocument = $oDocumentModel->getDocument($document_srl);
		if(!$oSourceDocument->isExists()) return new baseObject(-1,'msg_invalid_request');

		//if($oKinModel->getSelectedReply($document_srl)) return new baseObject(-1,'msg_invalid_request');
	...
	function procKinUpdateReply() {
		...
		if(!$oReply->isExists()) return new baseObject(-1,'msg_invalid_request');

		if($oKinModel->getSelectedReply($oReply->get('document_srl'))) return new baseObject(-1,'msg_invalid_request');
	...
	function procKinDeleteReply() {
		...
		$oReply = $oCommentModel->getComment($comment_srl);
		if(!$oReply->isExists()) return new baseObject(-1,'msg_invalid_request');
		if(!$oReply->isGranted()) return new baseObject(-1,'msg_not_permitted');

		$oSourceDocument = $oDocumentModel->getDocument($oReply->get('document_srl'));
		if(!$oSourceDocument->isExists()) return new baseObject(-1,'msg_invalid_request');

		if($oKinModel->getSelectedReply($document_srl)==$comment_srl) return new baseObject(-1,'msg_invalid_request');
	...
	function procKinInsertComment() {
		...
		if(!$this->module_srl || !$this->grant->write_reply) return new baseObject(-1,'msg_invalid_request');
		$logged_info = Context::get('logged_info');
		if(!$logged_info->member_srl) return new baseObject(-1,'msg_invalid_request');
	...
	function procKinDeleteComment() {
		...
		if(!$output->toBool() || !$output->data) return new baseObject(-1,'msg_invalid_request');

		$reply = $output->data;
		if($reply->module_srl != $this->module_srl || (!$this->grant->manager && $reply->member_srl != $logged_info->member_srl)) return new baseObject(-1,'msg_not_permitted');
